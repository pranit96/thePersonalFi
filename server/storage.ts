  ...
  async addAiInsights(insights: any[], userId?: number): Promise<void> {
    ...
    for (const insightData of insights) {
      try {
        ...
        const insightContent = insightData.content || 
                               insightData.description || 
                               insightData.text || 
                               "Financial insight generated by AI";
        
        // Make sure content is a string and not null or undefined
        if (typeof insightContent !== 'string' || !insightContent.trim()) {
          console.error("Skipping insight: content is empty or invalid");
          console.error("Problematic insight data:", insightData);
          continue; // Skip this insight
        }
        
        // Prepare the insight data, ensuring all required fields have values
        ...
        // Log the insight being inserted for debugging
        console.log(`Adding insight: ${JSON.stringify({
          id: newInsight.id,
          title: newInsight.title,
          content: newInsight.content.substring(0, 30) + '...',
        ...
        console.error(`Error inserting insight: ${error.message}`, error);
        console.error(`Problematic insight data:`, insightData);
      }
    }
  }
